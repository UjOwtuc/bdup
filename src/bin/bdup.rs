...
fn init_args_parser() -> clap::App<'static, 'static> {
    clap::App::new("bdup")
        ...
        .arg(clap::Arg::with_name("local_clients")
            .short("L")
            .long("local-clients")
        ...
        .arg(clap::Arg::with_name("config_file")
            ...
            .takes_value(true)
            .empty_values(false))
        .arg(clap::Arg::with_name("dump_config")
            .short("C")
            .long("dump-config")
...
fn main() {
    ...
    let config = read_config(&matches).unwrap_or_else(|err| { panic!("Could not parse config: {:?}", err); });
    if matches.is_present("dump_config") {
        ...
        println!("{}", serde_yaml::to_string(&config).unwrap_or_else(|err| panic!("Could not serialize config: {:?}", err)));
        return;
    }
    ...
    for conf in config.clients {
        ...
        client.find_local_backups(&PathBuf::from(&conf.storage_url)).unwrap_or_else(|err| log::error!("Could not find backups for client {}: {:?}", &conf.name, err));
        clients.push(client);
    }
...
mod test {
    ...
    fn arg_log_level_setting() {
        let res = init_args_parser().get_matches_from(vec![
            "bdup", "-l", "debug"
        ]);
    ...
    fn arg_log_level_validator() {
        let res = init_args_parser().get_matches_from_safe(vec![
            "bdup", "-l", "something"
        ]);
        assert!(res.is_err());
    }
    ...
    fn arg_config_clients() {
        let config = read_config(&init_args_parser().get_matches_from(vec![
                "bdup", "-c", "client_name=/some/comp=lex/path"
        ])).unwrap();
