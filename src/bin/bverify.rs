...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    fern::Dispatch::new()
        ...
        .level(log::LevelFilter::from_str(matches.value_of("log_level").unwrap())?)
        .chain(std::io::stdout())
        ...
        match Backup::new(&PathBuf::from(path)) {
            Ok(mut backup) => {
                if let Err(err) = backup.verify(num_threads) {
                    errors += 1;
                    log::error!("Verify of backup {} failed: {:?}", backup.path.display(), err);
                }
            },
            Err(err) => {
    ...
    if errors > 0 {
        Err(Box::new(VerifyError { errors, total: total_backups }))
    }
    else {
        Ok(())
    }
